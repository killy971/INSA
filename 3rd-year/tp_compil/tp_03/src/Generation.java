/**
 * Classe Generation : génère le code HTML de sortie 
...
class Generation extends Gram implements Constants {

	
	private OutputStream fichierSortie; // fichier html de sortie
	private ArrayList numerotation; // ArrayList pour numérotation des para
	private int tempLineSize; // taille de la ligne courante (en caractères)
	private int precBalisePara; // précédente balise de type para rencontrée

	public Generation (InputStream flot,String filename)
	{
		super(flot);
		fichierSortie = Ecriture.ouvrir(filename);
		numerotation = new ArrayList();
	 ...
	 */
	private int generationHNum(int t)
	{
		if (t<6)
		{
			return t+1;
	...
	protected void generationHTML()
	{
		...
		switch(token.unite)
		{
			...
			case IDENT:
				...
				if ((tempLineSize+=lex.identLu.length()+1)>60)
				{
					tempLineSize = 0;
