...
public class Ensemble {
	...
	public void ajoute2(int e) {
		...
		int[] new_tab = new int[card+1];
		int i;
		for (i=0 ; i<card && (e > tab[i]); i++) {
			new_tab[i] = tab[i];
		} //for
		new_tab[i] = e;
		for (; i<card ; i++) {
	...
	/**
	 * Lit une liste d'éléments au clavier qui seront ajoutés à l'ensemble
	 * remarque: ne marche apparemment pas lorsque lireUn() a déjà été executé
	 */
	public void lire() {
		String entree_clavier = Lecture.lireString();
		StringTokenizer st = new StringTokenizer(entree_clavier);
		while(st.hasMoreTokens()) {
			int entier = Integer.parseInt(st.nextToken());
			this.ajoute(entier);
		} // while
	} // lire()
	...
	public static void main(String args[]) {
		...
		Ensemble e7 = new Ensemble();
		Ensemble e8 = new Ensemble();
		...
		e7.lireUn();
		e7.affiche();
		System.out.println("Entrez plusieurs entiers.");
		e8.lire();
		e8.affiche();
	} // main()
